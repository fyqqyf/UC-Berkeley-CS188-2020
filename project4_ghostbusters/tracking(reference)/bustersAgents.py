...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            pos_i=dist.argMax()
            dist_i=self.distancer.getDistance(pacmanPosition,pos_i)
            if dist_i <= distance:
                distance=dist_i
                ghostpostion=pos_i
        ...
        dist=float('inf')
        for act in legal:
            pos_next=Actions.getSuccessor(pacmanPosition,act)
            dist_i=self.distancer.getDistance(pos_next,ghostpostion)
            if dist_i<dist:
                dist=dist_i
                action=act
        return action
